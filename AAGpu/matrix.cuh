...
template <unsigned Rows, unsigned Cols> struct matrix
{
    ...
    {
        static_assert((R >= Rows) && (C >= Cols), "Can only copy from the matrix \
            larger than current");
    ...
    template <unsigned R>
    __device__ matrix<1, Cols> rowSlice() const
    {
...
__inline__ __device__ void __colVec(matrix<Size, 1> &mat, T1 v1, T... args)
{
    mat(N, 0) = v1;
...
__inline__ __device__ void __colVec(matrix<Size, 1> &mat, T1 v1)
{
    mat(N, 0) = v1;
}

template <typename ...T> 
__inline__ __device__ matrix<sizeof...(T), 1> colVec(T... args)
{
